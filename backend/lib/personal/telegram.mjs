// Thin Telegram Bot API wrapper for the personal-calendar bot. Deliberately a separate token
// from lib/telegram.mjs (the kids' family bot) — two different Telegram bots, two different
// conversations. Loads the token from backend/.env automatically.
...
if (!process.env.TELEGRAM_PERSONAL_BOT_TOKEN && existsSync(ENV_PATH)) {
  try {
...
function token() {
  const t = process.env.TELEGRAM_PERSONAL_BOT_TOKEN;
  if (!t) throw new Error("TELEGRAM_PERSONAL_BOT_TOKEN missing — check backend/.env");
  return t;
}
