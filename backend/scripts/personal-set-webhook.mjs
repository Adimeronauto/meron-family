// Register / inspect / remove the personal bot's Telegram webhook.
//   node scripts/personal-set-webhook.mjs info
//   node scripts/personal-set-webhook.mjs set https://<app>.vercel.app/api/telegram-personal <secret>
//   node scripts/personal-set-webhook.mjs delete
...
if (!process.env.TELEGRAM_PERSONAL_BOT_TOKEN && existsSync(ENV)) process.loadEnvFile(ENV);
const token = process.env.TELEGRAM_PERSONAL_BOT_TOKEN;
...
if (cmd === "info") {
  ...
  const sec = secret || process.env.TELEGRAM_PERSONAL_WEBHOOK_SECRET;
